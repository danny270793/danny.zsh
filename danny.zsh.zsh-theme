...
prompt_context() {
  if [[ -n "$SSH_CLIENT" ]]; then
    ...
    prompt_segment yellow magenta "%{$fg_bold[magenta]%(!.%{%F{magenta}%}.)%}👤 $USER@$(hostname)%{$fg_no_bold[magenta]%}"
  fi
}
...
prompt_battery() {
  ...
  if [[ $(uname) == "Darwin" ]] ; then
    ...
    b=$(battery_pct)
      ...
      if [[ $(battery_pct_remaining) = "External Power" ]]; then
        echo -n "%{$fg_bold[white]%}$POWER_CORD$(battery_pct)%%%{$fg_no_bold[white]%}"
      else
...
prompt_status() {
  ...
  [[ -n "$symbols" ]] && prompt_segment red default "$symbols $RETVAL"
}
